...
simulate 3.minutes, {
  ...
  def n1 = node '1', remote: 1101, address: 1, location: [ 0.m, 0.m, -1.m], shell: true, stack:"$home/etc/initrc-stack"
     n1.startup = {
        ...
        router.send new RouteDiscoveryNtf(to: 3, nextHop: 3, reliability: false)
        router.send new RouteDiscoveryNtf(to: 4, nextHop: 3, reliability: false)

        add new TickerBehavior(7000, {
          println "\n\tData sent from 1"
 ...
 def n3 =  node '3', remote: 1103, address: 3, location: [0.m, 0.m, -800.m], shell: 5103, stack: "$home/etc/initrc-stack"
   n3.startup = {
        ...
        def router = agentForService Services.ROUTING
        router.send new RouteDiscoveryNtf(to: 2, nextHop: 2, reliability: false)
        router.send new RouteDiscoveryNtf(to: 1, nextHop: 1, reliability: false)
       // router.send new RouteDiscoveryNtf(to: 1, nextHop: 1)
        router.send new RouteDiscoveryNtf(to: 4, nextHop: 4, reliability: false)

        /*   add new TickerBehavior(11000, {
          println "\n\tData sent from 3"
          router << new DatagramReq(to: 4, data:[-1,-2,-3,-4])
          def txNtf = receive(TxFrameNtf, 1000)
 ...
 def n4 =  node '4', remote: 1104, address: 4, location: [0.km, 0.km, -1500.m], shell: 5104, stack: "$home/etc/initrc-stack"
   n4.startup = {
        ...
        def router = agentForService Services.ROUTING
        router.send new RouteDiscoveryNtf(to: 2, nextHop: 3, reliability: false)
        router.send new RouteDiscoveryNtf(to: 1, nextHop: 3, reliability: false)
        router.send new RouteDiscoveryNtf(to: 3, nextHop: 3, reliability: false)
